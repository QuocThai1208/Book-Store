...
class OrderSupplier(db.Model):
    ...
    unit_price_total = Column(Integer, nullable=False, default=0)
    order_supplier_details = relationship('OrderSupplierDetail', backref='OrderSupplier', lazy=True)
    ...
    def to_dto(self):
        return {
            ...
            'detail': [od.to_dto() for od in self.order_supplier_details]
        }


class OrderSupplierDetail(db.Model):
    id = Column(Integer, primary_key=True, autoincrement=True)
    order_supplier_id = Column(Integer, ForeignKey(OrderSupplier.id), nullable=False)
    book_id = Column(Integer, ForeignKey(Book.id), nullable=False)
    quantity = Column(Integer, nullable=False, default=1)
    unit_price = Column(Integer, nullable=False, default=0)
